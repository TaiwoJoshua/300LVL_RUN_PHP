<?php
    ...
    if(isset($matric) && $password !== ""){
        ...
        $select = "SELECT * FROM eee_admin_login_time WHERE matric='$matric'";
        ...
        $select = "SELECT * FROM eee_login_time WHERE matric='$matric'";
        $result = $conn->query($select);
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                $time = $_SESSION['time'] = $row['time'];
    ...
    $aready = "";
    $announce = array();
    $announcement = 'SELECT * FROM eee_announcement';
    ...
    $ttable = 'SELECT * FROM eee_timetable';
...
<head>
    ...
    <style>
        .view{
            background-image: linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url(../assets/img/nicolas-thomas-3GZi6OpSDcY-unsplash.jpg);
        }
    </style>
</head>
<body>
<form action="../assets/php/eee_update.php" method="POST" id="form" enctype="multipart/form-data">
    <div class="pagewrapper animate__animated animate__slower" id="pagewrapper">
        <div class="view" id="view">
            <!-- -------------------------------------- Navbar Section Starts Here -------------------------------------- -->
            <div id="navbar">
                <h1>RUN 300LVL ELECTRICAL ENGINEERING</h1>
            </div>
